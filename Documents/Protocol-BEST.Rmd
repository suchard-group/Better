...
<!--   1.1.0;2021-04-09;Methods to evaluate;Added historical comparator, cohort method, and SCCS variations;The historical comparator variation was based on initial EMAEUS results showing (preventable) outliers. The cohort method and SCCS variants are based on 3rd-party protocols for COVID-19 vaccine safety surveillance. -->
...
We will execute BETTER as part of an OHDSI network study, collaborating with OHDSI data partners. 
...
Table \@ref(tab:data-sources) lists the currently committed data sources for BETTER; these sources encompass a large variety of practice types and populations. 
...
Time-at-risk windows will be constructed both for the first and second dose, for vaccines (e.g., Zoster vaccine) with two doses. 
...
As we will conduct both estimation and testing tasks at the same time, we will compute two sets of metrics based on the estimation outcomes: 
metrics for testing, and metrics for estimation. (The following metrics are adapted from previous work [@Schuemie2020-wx].)
...
- The decision (signal or safety) made based on the $3 \times 3 = 9$ different combinations of decision rule thresholds
