...
**(need more editing to include all investigators)**
...
parties <- readr::read_delim(col_names = TRUE, delim = ";", trim_ws = TRUE, file = "
  ...
  Louisa Smith; Roux Institute, Northeastern University, Portland, ME, USA
...
To compare the real-data performance (precision, bias, testing errors, and timeliness) of
...
**(need further editing on the timeline)**
...
**(need further editing with updated information)**
...
Table \@ref(tab:data-sources) lists the 5 already committed data sources for BETTER; these sources encompass a large variety of practice types and populations. 
...
tab <- kable(data_sources, booktabs = TRUE, linesep = "",
      caption = "Committed BETTER data sources and the populations they cover.") %>% 
  kable_styling(bootstrap_options = "striped", latex_options = "striped") %>%
...
In this study, we mainly focus on two designs for counterfactual construction: historic comparator and self-controlled case series. 
...
#### Historic Comparator (HC)

Traditionally, vaccine surveillance methods compute an expected count based an incidence rate estimated during some historic time period, for example, in the years prior to the initiation of the surveillance study. 
We will use the historic period indicated in Table 8.1 and evaluate **four** variations:
...
- (For the historical comparator design) A log-normal prior with mean $=$ historic rate and SD $=4$ to incorporate historic information
