...
```{r plot bias distributions from EUMAEUS, fig.width=8, fig.height=6}
...
1. How to justify changing $h$? Is changing the hypothesis mid-way through the analysis a valid thing to do? (Note that in practice our analysis should be sequential.)

2. How to interpret Bayesian estimates (with/without adjustments) after the "calibration"?

   Our proposal: keep $h=0$, do bias adjustment, & calibrate on threshold $\delta_1$; then the posterior distribution *after* adjustment does produce interpretable estimates. 
